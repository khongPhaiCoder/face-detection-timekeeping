...
//
//this program has modified by wiwidnadw (Github Id)
//Tutor are share on YouTube Channel : Digitalneering
//Socmed line : wiwidnadw
//contact gmail direct contact into Github Repository
...
//initiate start position for servo
int servopositionx = 90;
int servopositiony = 20;


// Like arduino analogWrite
// value has to be between 0 and valueMax
void ledcAnalogWrite(uint8_t channel, uint32_t value, uint32_t valueMax = 180)
{
  // calculate duty, 8191 from 2 ^ 13 - 1, range 38 to 180. span 142
  // 8191/180 = about 45. span 142 = 6461
  // set duty to 6461 with zero at 38 x 45.5 = 1729
  uint32_t duty = (6462 / 180) * min(value, valueMax) + 1729 ;
...
static esp_err_t checkin_handler(httpd_req_t *req ){
    ...
    if (!fb) {
        Serial.println("Camera Checkin capture failed");
...
static esp_err_t checkout_handler(httpd_req_t *req ){
    camera_fb_t * fb = NULL;
    esp_err_t res = ESP_OK;
    int64_t fr_start = esp_timer_get_time();
    ...
    fb = esp_camera_fb_get();
    if (!fb) {
        Serial.println("Camera capture Checkout failed");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    if(!strcmp(variable, "framesize")) {
    ...
    else if(!strcmp(variable, "led_cmd")) {
      led_enabled = val;
      if (led_enabled) {
        digitalWrite (4, HIGH);
      } else {
      ...
      servopositionx = val;
      ledcAnalogWrite(2, servopositionx);
      ...
      servopositiony = val;
      ledcAnalogWrite(4, servopositiony);
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &checkout_uri);
    }
